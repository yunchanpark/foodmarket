...
public class ProductService {
	...
	public List<Product> productSelecet(SearchDTO searchDTO) {
		...
		// 검색 카테고리와 검색 키워드만 입력했을 때
			...
			// 검색 카테고리가 상품명일 때
			if(keyword.equals("name")) list = productRepository.findByNameContainsIgnoreCase(searchKeyword);
			// 검색 카테고리가 추가 상품명일 때
			if(keyword.equals("description")) list = productRepository.findByDescriptionContainsIgnoreCase(searchKeyword);
			// 검색 카테고리가 상품번호일 때
		...
		// 검색 카테고리와 날짜를 같이 입력했을 때
			...
			if(keyword.equals("name")) list = productRepository.findByNameContainsIgnoreCaseAndUpdatedAtBetween(searchKeyword, start, end);
			if(keyword.equals("description")) list = productRepository.findByDescriptionContainsIgnoreCaseAndUpdatedAtBetween(searchKeyword, start, end);
			if(keyword.equals("productNo")) list = productRepository.findByProductNoAndUpdatedAtBetween(Long.parseLong(searchKeyword), start, end);
		}
	...
	public List<Product> productByCategoryNo(Long categoryNo) {
		return productRepository.findByCategoryNo(
				productCategoryRepository.findById(categoryNo).get());
	}
